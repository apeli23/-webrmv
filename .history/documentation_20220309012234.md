...
We then populate the video element with the webcam stream and run the function named `transform` which will capture the canvas element's context and populate it using a draw 