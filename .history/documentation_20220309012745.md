...
We then populate the video element with the webcam stream and run the function named `transform` which will capture the canvas element's context and populate it with current video frame using a `drawImage` method. We then use the `getImageData`  